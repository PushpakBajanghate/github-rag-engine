...
st.set_page_config(
    page_title="GitHub Code & Issue RAG Engine", page_icon="🔍", layout="wide"
)
...
with st.sidebar:
    ...
    repo_url = st.text_input(
        "GitHub Repo URL", placeholder="https://github.com/psf/requests"
    )

    with st.expander("🛠️ Advanced Ingestion Settings", expanded=False):
        max_files = st.slider(
            "Max Files to Index", min_value=50, max_value=1000, value=300, step=50
        )
        max_issues = st.slider(
            "Max Issues to Index", min_value=0, max_value=200, value=50, step=10
        )
    ...
    if index_btn and repo_url:
        try:
            with st.spinner(
                "1. Fetching repository files, notebooks & issues in parallel..."
            ):
                raw_docs = fetch_repository_data(
                    repo_url, max_issues=max_issues, max_files=max_files
                )

            if not raw_docs:
                st.warning(
                    "No supported code files, notebooks, or issues found in this repository."
                )
            else:
                notebook_count = sum(
                    1 for d in raw_docs if d.metadata.get("type") == "notebook"
                )
                with st.spinner(
                    f"2. Chunking {len(raw_docs)} documents ({notebook_count} notebooks) with AST splitters..."
                ):
                ...
                with st.spinner(
                    f"3. Embedding and persisting {len(chunked_docs)} chunks to ChromaDB (rate-limit safe)..."
                ):
                    index_documents(chunked_docs, progress_callback=on_progress)
                status_placeholder.empty()
                st.success(
                    f"✅ Indexed {len(chunked_docs)} chunks from {len(raw_docs)} files (including {notebook_count} notebooks)!"
                )
        ...
        if "sources" in msg and msg["sources"]:
            with st.expander("📚 View Retrieved Sources"):
                for src in msg["sources"]:
                    st.markdown(
                        f"- **[{src.get('type', 'doc')}]** [{src.get('source', 'source')}]({src.get('url', '#')})"
                    )

if prompt := st.chat_input("Ask a question (in English, Hinglish)..."):
    ...
    with st.chat_message("assistant"):
        try:
            ...
            with st.expander("🧠 Query Normalization & Retrieval Insights"):
                if normalized:
                    st.markdown(
                        f"**Detected Language:** `{normalized.detected_language}` | **Tone:** `{normalized.response_tone}` | **Multi-intent:** `{normalized.is_multi_intent}`"
                    )
                    st.markdown(
                        f"**Context Resolved Query:** *{normalized.context_resolved_query}*"
                    )
